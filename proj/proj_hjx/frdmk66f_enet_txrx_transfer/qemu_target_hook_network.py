...
class QemuTarget(Target):
    ...
    def assemble_cmd_line(self):
        ...
        conf_absfile = r'/'.join([self.avatar.output_directory, self.QEMU_CONFIG_FILE])
        set_env = ['env', 'CONF_FILE=%s' % conf_absfile]
        # print(set_env)
        for term in self.hook_net:
            set_env += ['env', '{0}={1}'.format(term, self.hook_net[term])]

        gdb_option = ["-gdb", "tcp::" + str(self.gdb_port)]
        stop_on_startup = ["-S"]
        nographic = ["-nographic"]
        # monitor = ["-monitor", "/dev/null"]
        drive = ["-drive", "file="+str(self.drive_qcow2)+",if=none,format=qcow2"]
        qmp = ['-qmp', 'tcp:127.0.0.1:%d,server,nowait' % self.qmp_port]



        if self.drive_qcow2 == None:
            cmdl = executable_name + machine + debug_log + kernel + gdb_option \
                   + stop_on_startup + self.additional_args + nographic + qmp
        else:
            cmdl = executable_name + machine + debug_log + kernel + gdb_option \
                   + stop_on_startup + self.additional_args + nographic + qmp + drive

        if self.plugins != None:
            qemu_rootpath = r'/'.join(executable_name[0].split(r'/')[:-1])
            qemu_plugins = ['--tcg-plugin', r'/'.join([qemu_rootpath, 'tcg-plugin-hook_network.so'])]
            cmdl = set_env + cmdl + qemu_plugins
    ...
    def interrupt_injection(self, qmp, interval):
        ...
        while True:
            ...
            if True:
                ...
                # we have more than one peripherals
    ...
    def saveVMSnapshot(self, name):
        ...
        """
        filename = "%s/%s" % (self.avatar.output_directory, name)
        ...
        if TargetStates.STOPPED == self.get_status()['state']:
            ...
            ret = self.protocols.monitor.saveVM(filename)
            self.log.warning("-cc-> " + str(ret))
            self.log.warning("-cc-> save a vm snapshot, Done!")

    def loadVMSnapshot(self, name):
        """
        Load a vm snapshot
        """
        filename = "%s/%s" % (self.avatar.output_directory, name)
        self.log.warning("state: " + str(self.get_status()['state']))
        if TargetStates.STOPPED == self.get_status()['state']:
            self.log.warning("-cc-> load a vm snapshot!")
            ret = self.protocols.monitor.loadVM(filename)
            self.log.warning("-cc-> " + str(ret))
            self.log.warning("-cc-> load a vm snapshot, Done!")

    def infoVMSnapshot(self):
        """
        info snapshots
        """
        ret = "hjx"
        self.log.warning("state: " + str(self.get_status()['state']))
        if TargetStates.STOPPED == self.get_status()['state']:
            self.log.warning("-cc-> list all snapshots!")
            ret = self.protocols.monitor.infoVM()
            self.log.warning("-cc-> " + str(ret))
            self.log.warning("-cc-> list all snapshots, Done!")
        return ret
    ...
    def init(self):
        ...
        if self.interval == 0.01:
            # thread.start()	            # start a mutually exclusive thread with RMP
