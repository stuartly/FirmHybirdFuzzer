...
import proj.proj_hjx.frdmk66f_enet_txrx_transfer.firmConf as conf              #load firmware specific configuration
...
def run(q):
    ...
    # hook_network plugin args
    hook_net = {}
    hook_net["NET_FUN_ADDR"] =  funcs.get(conf.NET_FUNC, 0)
    hook_net["SEED_PATH"] =  os.path.join(conf.PROJ_PATH, "seed")
    hook_net["BUF_REG_INDEX"] = conf.BUF_REG_INDEX
    hook_net["LEN_REG_INDEX"] = conf.LEN_REG_INDEX
    hook_net["GVFILE_PATH"] = conf.GV_FILE
