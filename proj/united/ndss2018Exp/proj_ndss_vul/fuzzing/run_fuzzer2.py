...
HEURISTIC_BLACKLIST = set(['0800fd6c', '0800997a', '080114fe'])
...
class Panda_wycinwyc_procmon(External, object):
        ...
        if self.panda_log:
            ...
            if new_log_data:
                for line in self.panda_log.readlines():
                    ...
                    # self.cclines.append(line)
                    if line.startswith('[!]'):
                        if set(line.split()) & HEURISTIC_BLACKLIST:
                            return 0
                        self.crashed = True
                        #logger.debug("bug: " + line)
                        #for i in range(1, 60, 1):
                        #    logger.debug("out: " + self.cclines[i-60])
                        #del self.cclines[:]
                        return -1
        return 0
    ...
    def post_send(self):
        logger.debug("Panda_wycinwyc_procmon + post_send")
        self.num_fuzzed += 1

        if self.crashed:
            logger.debug("Panda_wycinwyc_procmon + post_send + crash")
            self.crashed = False
            self.num_crashed += 1
            return False
        return True
    ...
    def stop_target(self):
        logger.debug("Panda_wycinwyc_procmon + stop_target")
        return True

    def start_target(self):
        logger.debug("Panda_wycinwyc_procmon + start_target")
        if self.cccxxx == 0:
            # exit(1)
            subprocess.call(["touch", "/doit/firmware/test/proj_ndss_vul/RESTART"])
            logger.debug("-cc-> start new target <<<<Done<<<")
            sleep(1)
        else:
            self.cccxxx -= 1
    ...
    def restart_target(self):
        logger.debug("Panda_wycinwyc_procmon + restart_target")
        self.stop_target()
...
def target_recv_until_oend(target, max_bytes):
    ...
    while data[-5:] != 'OEND\n' and len(data) <= max_bytes:
        ...
        if start_time and time() - start_time >= target._target_connection._recv_timeout:
            break
        data += byte
...
def main(port=9998, timeout=15, duration=3600, corruption_probability=0.1, verbose=True):
    ...
    def select_file_cb(target, log, session, node, edge):
        ...
        logger.debug("-------------> send " + input_names[n] )
        return inputs[n]
        # return inputs[0]
    ...
    fuzzing_session = sessions.Session(target=target, crash_threshold_request=300, crash_threshold_element=3000, receive_data_after_fuzz=True,
                                       # sleep_time=5.0
                                       )

    # fuzzing_session.post_send = session_post_send
    fuzzing_session.register_post_test_case_callback(session_post_send)
