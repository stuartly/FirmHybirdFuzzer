...
/*
static void gdb_vm_state_change_flex(void *opaque, int running, RunState state)
{
...
static void gdb_vm_state_change(void *opaque, int running, RunState state)
{
    ...
    int ret;
    //int pc;
    //gdb_read_register(cpu, (uint8_t *)&pc, 15);
    if (running || s->state == RS_INACTIVE) {
        return;
    }
...
uint64_t get_current_pc(void){
    uint32_t pc;
    tb_flush_now(gdbserver_state->g_cpu);

    //cpu_handle_guest_debug(gdbserver_state->c_cpu);
    //gdb_continue(gdbserver_state);
    //cpu_synchronize_state(gdbserver_state->g_cpu);
    vm_stop(RUN_STATE_DEBUG);
    gdb_vm_state_change_flex(0, 0, RUN_STATE_DEBUG);
    gdb_read_register(gdbserver_state->g_cpu, (uint8_t *)&pc, 15);
    gdb_continue_partial(gdbserver_state, "c");
    //vm_start();
    //pc = ARM_PC_FLEX;
    return (uint64_t)pc;
}
