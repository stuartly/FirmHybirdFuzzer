                                     ...
                                     MemTxAttrs attrs)
{
    ...
    case 0x280 ... 0x2bf: /* NVIC Clear pend */
        startvec = 8 * (offset - 0x280) + NVIC_FIRST_IRQ; /* vector # */
        printf(" -cc-> s->num_irq: %d\n", s->num_irq);

        for (i = 0, end = size * 8; i < end && startvec + i < s->num_irq; i++) {
            if (value & (1 << i) &&
                (attrs.secure || s->itns[startvec + i])) {
                s->vectors[startvec + i].pending = setval;
                printf(" -cc-> Interrupt: %d\n", startvec + i);
            }
        }
        nvic_irq_update(s);
        return MEMTX_OK;
    ...
    case 0x400 ... 0x5ef: /* NVIC Priority */
        ...
        for (i = 0; i < size && startvec + i < s->num_irq; i++) {
            if (attrs.secure || s->itns[startvec + i]) {
                //this is for nxp k66 workaround
...
static Property props_nvic[] = {
    ...
    DEFINE_PROP_UINT32("num-irq", NVICState, num_irq, 124),
...
static void __NVIC_SetPendingIRQ(int64_t IRQn, struct ISPR_INFO *info)
{
    //ispr is at 512
    info->addr = 0x200 + ((((uint32_t)IRQn) >> 5UL) << 2UL);
    info->value = 1UL << (((uint32_t)IRQn) & 0x1FUL);

}
static bool is_irq_enabled(){
    ARMCPU *cpu = ARM_CPU(qemu_get_cpu(0));
    uint32_t primask = cpu->env.v7m.primask[M_REG_NS];
    return primask == 0;
}

static ActiveIRQs * __qmp_active_irqs()
{
    ActiveIRQs * ret;
    MemTxAttrs attrs;
    ...
    if(!is_irq_enabled()){
        ...
        uint64_t data;
        ARMCPU *cpu = ARM_CPU(qemu_get_cpu(0));
        void *nvic = cpu->env.nvic;
        nvic_sysreg_read(nvic, 0x100, &data, 4, attrs);
        ret->v0 = data;
        nvic_sysreg_read(nvic, 0x104, &data, 4, attrs);
        ret->v1 = data;
    }
#endif
    //gdb_breakpoint_insert(0x50, 2, 0);
    return ret;
}

ActiveIRQs * qmp_active_irqs(Error **errp)
{
   return  __qmp_active_irqs();
}

void qmp_inject_interruption(int64_t irq, Error **errp)
{
    MemTxAttrs attrs;
    struct ISPR_INFO info;
    memset((void *)&attrs, 0, sizeof(attrs));
    if (irq < 0) {
        error_setg(errp, QERR_INVALID_PARAMETER_VALUE, "irq", "a size");
        return;
    }
    //do stuffs
    //
	...
	ARMCPU *cpu = ARM_CPU(qemu_get_cpu(0));
    void *nvic = cpu->env.nvic;
    __NVIC_SetPendingIRQ(irq-NVIC_FIRST_IRQ, &info);
    nvic_sysreg_write(nvic, info.addr, info.value, 4, attrs);
    qemu_log_mask(LOG_UNIMP, "inject %d\n", irq);
#endif
}

extern uint32_t remaining_sym;
void qmp_inject_interruption_all(Error **errp)
{
    MemTxAttrs attrs;
    struct ISPR_INFO info;
    int i;
/*
    if(remaining_sym){
        qemu_log_mask(LOG_UNIMP, "hold interrupt in injection-all remaining: %u\n", remaining_sym);
        return;
    }
*/
    memset((void *)&attrs, 0, sizeof(attrs));
    ...
    */
	...
	ARMCPU *cpu = ARM_CPU(qemu_get_cpu(0));
    void *nvic = cpu->env.nvic;
    ActiveIRQs * active_irqs = __qmp_active_irqs();
    for(i=0;i<32;++i){
        if((uint32_t)(active_irqs->v0) & 1 << i){
            if(!remaining_sym){
                __NVIC_SetPendingIRQ(i, &info);
                nvic_sysreg_write(nvic, info.addr, info.value, 4, attrs);
                qemu_log_mask(LOG_UNIMP, "inject %d\n", i);
            }else
                qemu_log_mask(LOG_UNIMP, "hold interrupt in injection-all remaining: %u\n", remaining_sym);
        }
        if((uint32_t)(active_irqs->v1) & 1 << i){
            if(!remaining_sym){
                __NVIC_SetPendingIRQ(i+32, &info);
                nvic_sysreg_write(nvic, info.addr, info.value, 4, attrs);
                qemu_log_mask(LOG_UNIMP, "inject %d\n", i+32);
            }else
                qemu_log_mask(LOG_UNIMP, "hold interrupt in injection-all remaining: %u\n", remaining_sym);
...
void qmp_set_vtor(int64_t vtor, Error **errp)
{
    MemTxAttrs attrs;
    //do stuffs
    //
	...
	ARMCPU *cpu = ARM_CPU(qemu_get_cpu(0));
    void *nvic = cpu->env.nvic;
    nvic_sysreg_write(nvic, 0xd08, vtor, 4, attrs);
    qemu_log_mask(LOG_UNIMP, "set vtor to 0x%x\n", vtor);
#endif
}
