                                 ...
                                 MemTxAttrs attrs, uint8_t *buf, int len)
{
    ...
    l = len;
    mr = flatview_translate(fv, addr, &addr1, &l, false, attrs);
    // printf("-cc-> flatview_read: addr1: 0x%lx\n", addr1); // rm useless printf -- hjx
...
// copy from panda, declaration in  -- hjx
// buf the implementation of address_space_translate() is different.
...
uint8_t * cpu_get_host_addr(AddressSpace *as, hwaddr addr){
    hwaddr l;
    uint8_t *ptr;
    hwaddr addr1;
    MemoryRegion *mr;

    mr = address_space_translate(as, addr, &addr1, &l, true, MEMTXATTRS_UNSPECIFIED);
