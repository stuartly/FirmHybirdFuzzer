...
def get_symbols(filename):
    ...
    with open(filename, 'rb') as f:
        ...
        for symbol in sym_sec.iter_symbols():
            if symbol['st_info']['type'] == 'STT_FUNC' or symbol.name == 'rtc_inited':
...
def hookSkippedCode(state):
    ...
    if state.addr >= 0xffff0000:
        logger.debug('-cc-> Interrupt place, No hook, return directly')
        return
...
def in_fixedPV(addr, pc):
    if addr in PMRSettings.fixedPeriV:
        if isinstance(PMRSettings.fixedPeriV[addr], dict):
            if pc in PMRSettings.fixedPeriV[addr]:
                logger.debug("-------> pc: " + format(pc, "#04x") + ", addr: " + format(addr, "#04x"))
                return PMRSettings.fixedPeriV[addr][pc]
        else:
...
def isForQemu(state):
    ...
    if stateInList(state, PMRSettings.stopHooks):
        logger.debug('[+] transfering to qemu due to stopHooks')
        return True
...
class Alg():
    ...
    def select_path_explore_single(self, sm):
        ...
        for s in sm.active:
            ...
            if detect_deadLoop(s, self.root_state):
                ...
                logger.debug(str(s) + "1-ccc-> -- direct loop detected")
                continue
            if in_asserts(s):
                statePath[s.addr].append([])
                ret.extend(statePath[s.addr])
                logger.debug(str(s) + "2-ccc-> --  assert detected")
                continue
        ...
        # min scores
        if len(min_scores) == 1:
            logger.debug("3-ccc-> select " + format(min_scores[0]) + " due to similarity check")
        ...
        logger.debug("# of selection: " + format(len(selection)))
        ...
        if self.cpsr & 0x01FF >= 16:  # interrupt is true
            ...
            if len(selection) == 1:
                ...
                logger.debug("4-ccc-> select " + format(index_r) + " due to higher addr")
    ...
    def branchSelection(self, x, sp, addrs):
        ...
        i = 0
        for addr in addrs:
            assert (addr == addrs[i]), "addr: " + format(addr, "#04x") + " addrs[i]: " + format(addrs[i],
                                                                                                "#04x") + " i: " + str(
                i)
            ll += len(sp[addr])
            if x < ll:
    ...
    def process(self):
        ...
        selection = manualSelectState(self.sm, PMRSettings.manual_selection)
        if selection != 100:
            logger.debug("-cc-> manual selection: " + selection)
            return selection
...
def store_regs(PATH, rr):
    regs = get_registers(rr)
    with open(PATH + '/regs.json', 'w') as f:
         f.write(str(regs))
