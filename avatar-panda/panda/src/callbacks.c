...
bool panda_load_plugin(const char *filename, const char *plugin_name) {
    ...
    fprintf(stderr, "panda_load_plugin: Load plugin! %s\n", plugin_name);
    uint32_t i;
    ...
    if(!init_fn(plugin) || panda_plugin_load_failed) {
        fprintf(stderr, "panda_load_plugin: FAILED to load plugin: %s\n", plugin_name);
        dlclose(plugin);
        return false;
    }
    fprintf(stderr, "panda_load_plugin: SUCCEED to load plugin: %s\n", plugin_name);
    return true;
}
