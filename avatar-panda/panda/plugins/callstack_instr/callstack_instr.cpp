...
uint32_t get_callers(target_ulong callers[], uint32_t n, CPUState* cpu) {
    ...
    //printf("\n++++++++++\n");
    //printf("callback size: 0x%x\n", (uint32_t)v.size());
    uint32_t xx = (uint32_t)v.size();
//    for (uint32_t j = 0; j < xx; j++) {
//        printf("call stacks: 0x%08x\n", v[j].pc);
//    }
//    printf("----------\n");
    if (xx == 0)
        xx += 1;
    for (uint32_t i=0; i<n; i++) { callers[i] = v[xx-i-1].pc; }
    return n;
}
