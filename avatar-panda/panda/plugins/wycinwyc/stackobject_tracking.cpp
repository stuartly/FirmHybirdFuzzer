...
target_ulong find_frame_by_address2(target_ulong addr){
    ...
    //printf("find_frame_by_address2======> callframes.size(): 0x%lx, addr: 0x%x\n", callframes.size(), addr);
    ...
    //printf("find_frame_by_address2222222======> callframes.size(): 0x%lx, addr: 0x%x\n", callframes.size(), addr);
    ...
    //printf("find_frame_by_address33333======> callframes.size(): 0x%lx, addr: 0x%x\n", callframes.size(), addr);
    return 0;
}
...
int after_block_exec_stackobject_cb(CPUState *cpu, TranslationBlock *tb){
    ...
    //printf("after_block_exec_stackobject_cb======> callframes.size(): 0x%lx\n", callframes.size());
...
int phys_mem_write_stackobject_cb(CPUState *cpu, target_ulong pc, target_ulong addr, target_ulong size, void *buf){
    ...
    //printf("phys_mem_write_stackobject_cb======> callframes.size(): 0x%lx\n", callframes.size());
...
void on_call_stackobject_cb(CPUState *cpu, target_ulong pc){
    ...
    if (func){
        ...
        if (stack_vars){
            QLIST_FOREACH_ENTRY(stack_vars, entry){
                ...
                //printf("-cc-> before callframes[sp] = stack_objects: 0x%lx\n", stack_objects.size());
    ...
    for(cf = callframes.begin(); cf != callframes.end(); ++cf) 
        printf("\t%x\n", cf->first);
#endif
    //printf("on_call_stackobject_cb======> callframes.size(): 0x%lx\n", callframes.size());
}
