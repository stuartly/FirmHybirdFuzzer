                                              ...
                                              QList *properties)
{
    ...
    printf("-cc-> make_configurable_device: address: 0x%" PRIx64 "\n", address);
    sysbus_mmio_map(s, 0, address);
    irq = qemu_allocate_irq(dummy_interrupt, dev, irqnum);
...
static void init_peripheral(QDict *device)
{
    ...
    QDICT_ASSERT_KEY_TYPE(device, "size", QTYPE_QINT);
    ...
    size = qdict_get_int(device, "size");
    name = qdict_get_str(device, "name");

    printf("Configurable: Adding peripheral[%s] region %s at address 0x%" PRIx64 ", size 0x%" PRIx64 "\n", 
            qemu_name, name, address, size);
...
static void board_init(MachineState * ms)
{
    ...
    qemu_register_reset(armv7m_reset, CPU(cpuu));
}
