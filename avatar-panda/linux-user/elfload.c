...
extern abi_ulong afl_entry_point, afl_start_code, afl_end_code;  //afl-qemu-patch
                           ...
                           char bprm_buf[BPRM_BUF_SIZE])
{
    ...
    if (!afl_entry_point) afl_entry_point = info->entry;   //afl-patch
        ...
        if (eppnt->p_type == PT_LOAD) {
            ...
            if (elf_prot & PROT_EXEC) {
                if (vaddr < info->start_code) {
                    ...
                    if (!afl_start_code) afl_start_code = vaddr;   //afl-patch
                }
                if (vaddr_ef > info->end_code) {
                    info->end_code = vaddr_ef;
                    if (!afl_end_code) afl_end_code = vaddr_ef;     //afl-patch
                }
            }
