...
class IgnorePeripheral(AvatarPeripheral):
    ...
    def solve_path(self, simgr, pc, angrPath):
        ...
        logger.debug("[+] consume one sym at addr: " + format(fetch_v[0], '#04x') + " .#" + format(
            len(utils.PMRSettings.global_v)))
    ...
    def hw_read(self, offset, size, pc, regs):
        try:
            fixed = utils.in_fixedPV(self.address + offset)
            if fixed:
                return fixed
            ...
            while len(utils.PMRSettings.global_v) != 0:
                ...
                if fetch_v[1] & ~1 == pc & ~1:
                    ...
                    return ret_v
                logger.debug(
                    "-cc-> Step through global_v! fetch_v[1]: " + format(fetch_v[1], "#04x") + ", pc: " + format(pc,
                                                                                                                 "#04x"))
    ...
    def nop_write(self, offset, size, value, pc, regs):
        ...
        logger.debug("IGN: " + self.name + " write at: " + format(self.address + offset, '#04x') +
                     "(" + format(offset, '#04x') + "), size: " + format(size, '#04x') + ", value: " + format(value,
                                                                                                              '#04x')
                     + ", pc: " + format(pc, '#04x'))
