...
class Avatar(Thread):
    ...
    def __init__(self, arch=ARM, output_directory=None):
        ...
        # rm shared memory created by QEMU
        self.purge("/dev/shm/", "SHM")

        # rm logging
        #self.purge(output_directory, "(.*?)")
    ...
    def add_memory_range(self, address, size, name=None, permissions='rwx',
                         ...
                         target=None,
                         emulate=None, alias = None, **kwargs):
        """
        ...
        if emulate:
            ...
            self.python_peripherals.append(python_peripheral)
        ...
        if alias is not None:
            m_alias = MemoryRange(alias, size, name=name+'alias', permissions=permissions,
                        file=file, forwarded=forwarded,
                        forwarded_to=forwarded_to, alias = True, **kwargs)
            self.memory_ranges[alias:alias + size] = m_alias
        return m
    ...
    def _handle_remote_memory_read_message(self, message):
        ...
        try:
            mem = range.forwarded_to.read_memory(message.address, message.size, message.pc, message.regs)
            success = True
        except:
            mem = -1
            success = False
    ...
    def _handle_remote_memory_write_msg(self, message):
        ...
        success = mem_range.forwarded_to.write_memory(message.address, message.size,
                                                      message.value, message.pc, message.regs)
    ...
    def run(self):
        ...
        while True:
            if self._close.is_set():
                break
            qemu=self.get_target('QemuTarget0')
            # qemu.mutex.acquire()
            # print("lock acquired by avatar")
            try:
                message = self.queue.get(timeout=0.5)
            except:
                # print("lock released by avatar")
                # qemu.mutex.release()
                continue
